...
- **Native Messages**: ArkUI `List`, `Search`, `TextInput`, HarmonyOS Symbols, padded conversation rows, native back/title state, and an in-conversation composer. The list is **the account's real inbox** — a bridge (`dmInboxScript`) runs inside the x.com origin so the session cookies apply, reads the inbox, and the shell polls it only while Messages is on screen. Nothing is seeded: a signed-out or empty account says so. A conversation opens as a real **push** (`TransitionEffect.move`, direction-aware so it mirrors in Arabic), the shell's tab bar steps aside while you are in a thread, rows carry native **swipe actions** (mute / archive), the filter is a frosted menu, and the composer has an emoji panel, an attachment chip and a send control that greys out when there is nothing to send. New inbound messages raise a native **notification** that brings the app back to the front
